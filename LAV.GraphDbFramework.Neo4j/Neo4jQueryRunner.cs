...
public sealed class Neo4jQueryRunner : BaseGraphDbQueryRunner<Neo4jRecord>
{
    private readonly IAsyncQueryRunner _runner;
    private static readonly ConcurrentDictionary<Type, Func<Neo4jRecord, object>> MapperCache = [];

    public Neo4jQueryRunner(IAsyncQueryRunner runner, Microsoft.Extensions.Logging.ILogger logger) : base(logger)
    {
        _runner = runner;
    ...
    public override async ValueTask<IReadOnlyList<T>> RunAsync<T>(string query, object? parameters)
    {
        return await RunAsync(query, parameters, record =>
        {
            ...
            {
                var mapperFunc = (Func<Core.IGraphDbRecord, T>)Delegate.CreateDelegate(typeof(Func<Neo4jRecord, T>), method);
